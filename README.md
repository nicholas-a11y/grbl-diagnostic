...
The GCOM scripting engine was added later and goes much further. It is inspired by the BASIC (Beginner's All-purpose Symbolic Instruction Code) language, and allows you to write interactive test routines, easily create code to tune machine settings, run motion benchmarks, save and share those scripts with others via a URL.

This tool can also support creative machining workflows that use AI prompts, since it acts as a bridge between software and machine motion, for example here is a script generated by ai for making a nice pattern with a laser:
https://gcomposer.app/?gcom=96ohek
...
Open the GCOM SCRIPT COMPOSER area to create or import new scripts, and for help & examples.

The connection itself can optionally be a part of the script.
...
```text
; TITLE: X-Axis Max travel Speed and Acceleration Finder
; VERSION: 1
; AUTHOR: GitHub Copilot
; DESCRIPTION: Raises X acceleration while sweeping fast X travel moves to find the highest reliable travel speed.
; DESCRIPTION: Suggests a conservative fallback near the edge and reports estimated midpoint travel speed.
; VAR start_accel=200
; VAR coarse_step=50
...
- Only top-of-file `;` header lines are treated as import metadata.
- Repeated `; DESCRIPTION:` lines become a multiline description.
- `; VAR name=value` defines imported variable defaults.
...
The "Save Script" button saves the script to the server and assigns it an ID, and copies the link immediately to the system clipboard.
